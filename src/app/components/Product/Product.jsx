...
export default function Product({addItemToSessionStorage, item, getProductToCart, getProduct, showElements, handleHeart}) {
          ...
          <div className='flex justify-center gap-2'>
            {
              colorArray.current.map((color,index)=>{ 
                return <div key={index} id={color} onClick={handleClickImage} style={{height: "1rem", width: "1rem", backgroundColor: color, borderRadius: "5rem", border: "1px solid grey", cursor: "pointer"}}></div> })
            }
          </div>
          { (showElements)?
          <div className='flex justify-evenly gap-2 items-center px-5 text-sm'>
            <div>{item.price} lei </div>
              <div className='flex justify-between gap-1'>
                <button onClick={(e) => handleHeart(e, item)}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 hover:fill-red-500 hover:scale-125">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                  </svg>
                </button>
                <button onClick={()=>addItemToSessionStorage(item)} className='bg-btn-color px-1 py-1 rounded-[0.5rem] hover:scale-125'>
