...
const Page = () => {
  ...
  {
  const fetcher = async (url) => await axios.get(url).then((res) => res.data);
  const { data, error } = useSWR(`/api/getAllProdPag?colors=${colorsArray}&brands=${brandsArray}&types=classic&minPrice=${minPrice}&maxPrice=${maxPrice}&sortPrice=${sortPrice}`, fetcher);     
    ...
    return (
        <div className='pb-8 w-full flex justify-center items-center flex-col bg-white'>
            <div className='w-full h-12 pl-36 pt-2 bg-[#F9F6F2]'>Classic Phones</div>
            <div className='w-full lg:pl-36 lg:pr-24 md:pl-36 md:pr-24 sm:px-16 pt-12 flex item-start text-base '>
                ...
                <div className='w-full flex-col w-3/4'>
                  ...
                  <div className='flex justify-center last-of-type:justify-start flex-wrap gap-8 pl-4'>
                    <Products setSelectedData={setSelectedData} limit={count} brands={brandsArray} colors={colorsArray} types='classic' minPrice={minPrice} maxPrice={maxPrice} sortPrice={sortPrice}/>
                    <div style={{ display: 'none' }}>
                      <Products setSelectedData={setSelectedData} limit={count + 4} brands={brandsArray} colors={colorsArray} types='classic' minPrice={minPrice} maxPrice={maxPrice} sortPrice={sortPrice}/>
                    </div>
                  </div>
