...
import data from '../cart/components/data.json'
const useSetPrice = () => {
    ...
    const reducer = (state, action) => {
        ...
        // console.log(price, action, selectedMemory, selectedRam);
        switch (action.type) {
            case 'count': 
                switch (action.value) {
                    case '-':
            ...
            case 'setSelectedMemory':
                ...
                console.log('valoarea selectata', value);
                return {
                  ...state,
            ...
            case 'setValueMemory':
                ...
                console.log(selectedMemory, updatedSelectedMemory);
            if (action.value === updatedSelectedMemory && updatedSelectedMemory !== previousSelectedRefMemory.current) {
                ...
                if (previousSelectedRefMemory.current != updatedSelectedMemory) {
                     newPrice = (price - (price - data.price * counter))  ; 
                    console.log(newPrice);
                    newPrice = newPrice + Number(action.id) * 500 * counter
                } else {
                    let dif = price - data.price * counter ; console.log(dif);
                    newPrice = (price - (price - data.price * counter)) + dif ; 
                    console.log(newPrice);
                    newPrice = newPrice + Number(action.id) * 500 * counter
                }                    
...
useEffect(() => {
  ...
  return {state, handleCounter, handleMemory, handleRam}
